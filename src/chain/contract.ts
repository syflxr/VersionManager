...
import { getLogger, ILogger } from "../sdk-v3";
...
export class Contract {
  ...
  public static async deployWithLib(contractName: string, libraries: {}, signer: Signer, args: any[], log?: {
    ...
    if (log) {
      ...
      console.log("contract-deploy",
        ...
        "hash:", contract.deployTransaction.hash,"gasUsed:",Number(receipt.gasUsed));
  ...
  public static async deployProxyWithLib(contractName: string, libraries: {}, signer: Signer, args: any[], log?: {
    ...
    let proxy = <TransparentUpgradeableProxy>await Contract.deploy("TransparentUpgradeableProxy", signer, [impl.address, proxyAdmin ?? await signer.getAddress(), data, ...args.slice(-1)], log);
    // TODO optimize
    return new EthersContract(proxy.address, impl.interface, signer);
  }

  /**
   * @param contractName 同deploy方法
   * @param signer 同deploy方法
   * @param args 穿实现合约的参数，初始化使用
   * @param log 同deploy方法
   * @param proxyAdmin 代理合约的管理员，只能调用代理合约，不能调用实现合约，若为signer地址，则signer无法调用实现合约
   * @param impl 实现合约，不传自动部署一个
   * @param initializer 穿出事化方法名，不传默认为initialize，也可穿false表示不在部署时初始化合约
   */
  ...
  public static async get(contractName: string, contractAddress: string, signer: Signer, log?: { name: string }) {
    ...
    if (log) {
      console.log("contract-get", "logName", log.name, "contractName", contractName, "contractAddress", contract.address, "signer", await signer.getAddress());
    }
    return contract;
  }
  ...
  public static async getDeployment(chainId: string, contractName: string, signer: Signer, log?: { name: string, folder: string, saveName: string }, proxy: boolean = false) {
    ...
    try {
      ...
      if (log) {
        console.log("contract-getByDeployment", "contractAddress", deployment.contractAddress, "name", saveName, "signer", await signer.getAddress());
      }
    } catch (e) {
      console.log("contract-getByDeployment-not exist", "contractName", contractName, "path", path);
      return null;
    }
